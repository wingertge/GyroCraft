package gyrocraft.client.render.tileentity;

import com.mojang.blaze3d.matrix.MatrixStack;
import gyrocraft.config.GyroCraftConfig;
import gyrocraft.tileentity.ElectricFurnaceTileEntity;
...
public class ElectricFurnaceTileEntityRenderer extends TileEntityRenderer<ElectricFurnaceTileEntity> {
	...
	public void render(final ElectricFurnaceTileEntity tileEntityIn, final float partialTicks, final MatrixStack matrixStack, final IRenderTypeBuffer renderTypeBuffer, final int packedLight, final int backupPackedLight) {
		...
		final boolean hasEnergy = tileEntityIn.energy.getEnergyStored() >= GyroCraftConfig.electricFurnaceEnergySmeltCostPerTick;
		...
		Minecraft.getInstance().getBlockRendererDispatcher().renderBlock(renderState, matrixStack, renderTypeBuffer, packedLight, backupPackedLight, tileEntityIn.getModelData());
	}
